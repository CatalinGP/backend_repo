...
def manual_send_frame(can_id, can_data):
    ...
    try:
        log_info_message(
            logger,
            f"Attempting to connect to CAN bus on channel: {Config.CAN_CHANNEL}")
        bus = bridge.get_bus()
        ...
        log_info_message(
            logger,
            f"Parsed CAN ID: {hex(can_id)}, CAN Data: {[hex(b) for b in can_data]}")
        ...
        log_info_message(
            logger, f"Sending CAN message: ID={hex(message.arbitration_id)}, \
        ...
        while True:
            ...
            if received_frame is None:
                log_info_message(
                    logger, "No more frames received, exiting receive loop")
                break

            log_info_message(
                logger, f"Received frame: ID={hex(received_frame.arbitration_id)}, \
            ...
            # Handle negative response codes
                ...
                if nrc == 0x37:
                    time_delay_ms = int.from_bytes(
                        received_frame.data[4:8], byteorder='big')
                    time_delay_s = time_delay_ms / 1000
                    log_warning_message(
                        logger, f"Retries exceeded. Try again in: {time_delay_s} s")
