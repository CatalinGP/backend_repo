...
class TestAPISendFrame(TestCase):
    ...
    def test_send_frame_success_clear_diagnostic(self, mock_manual_send_frame):
        ...
        mock_manual_send_frame.return_value = {"response": [
            {"can_data": ["0x1", "0x54"], "can_id": "0X11FA"}]}
...
class TestAPIs(unittest.TestCase):
    ...
    def test_request_ids_success(self, mock_get_request_id):
        ...
        self.assertEqual(
            ...
            "The HTTP response is not the expected one")
        self.assertEqual(
            response.json['ecus'][0]['ecu_id'],
            fake_json['ecus'][0]['ecu_id'],
            "ECU ID is incorrect")
        self.assertEqual(
            response.json['ecus'][0]['version'],
            fake_json['ecus'][0]['version'],
            "ECU version is incorrect")
        self.assertEqual(
            response.json['mcu_id'],
            fake_json['mcu_id'],
            "MCU ID is incorrect")
        self.assertEqual(
        ...
        # Assert using not Equal because it is not possible to assert time_stamp
        # with the same values in isoformat"
        self.assertNotEqual(
            response.json['time_stamp'],
            fake_json['time_stamp'],
