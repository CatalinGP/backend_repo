...
PROJECT_SRC_PATH = os.path.abspath(os.path.join(os.getcwd(), "..", ".."))
PATH_SOFTWARE_RELEASES = os.path.join(os.path.dirname(PROJECT_SRC_PATH), 'software_releases')
    ...
    try:
        ...
        if sw_to_build == "mcu" or sw_to_build == "all":
            if "clean" in extra_args:
                subprocess.run(
                    ["sudo", "make", "clean", "-C", "{0}/backend/mcu/".format(PROJECT_SRC_PATH)], check=True)
            # Build the MCU executable
                subprocess.run(
                    ["sudo", "make", f"SOFTWARE_VERSION={version_integer}", "-C", "{0}/backend/mcu/".format(PROJECT_SRC_PATH)], check=True)
            mcu_executable_path = "{0}/backend/mcu/main_mcu".format(
                PROJECT_SRC_PATH)
        ...
        if sw_to_build == "battery" or sw_to_build == "all":
            if "clean" in extra_args:
                subprocess.run(
                    ["sudo", "make", "clean", "-C", "{0}/backend/ecu_simulation/BatteryModule".format(PROJECT_SRC_PATH)], check=True)
            # Build the Battery Module executable
            subprocess.run(
                ["sudo", "make", f"SOFTWARE_VERSION={version_integer}", "-C", "{0}/backend/ecu_simulation/BatteryModule".format(PROJECT_SRC_PATH)], check=True)
            battery_executable_path = "{0}/backend/ecu_simulation/BatteryModule/main_battery".format(
                PROJECT_SRC_PATH)
        ...
        if sw_to_build == "engine" or sw_to_build == "all":
            ...
            subprocess.run(
                ["sudo", "make", f"SOFTWARE_VERSION={version_integer}", "-C", "{0}/backend/ecu_simulation/EngineModule".format(PROJECT_SRC_PATH)], check=True)
            engine_executable_path = "{0}/backend/ecu_simulation/EngineModule/main_engine".format(
                PROJECT_SRC_PATH)
        ...
        if sw_to_build == "doors" or sw_to_build == "all":
            if "clean" in extra_args:
                subprocess.run(
                    ["sudo", "make", "clean", "-C", "{0}/backend/ecu_simulation/DoorsModule".format(PROJECT_SRC_PATH)], check=True)
            # Build the doors Module executable
            subprocess.run(
                ["sudo", "make", f"SOFTWARE_VERSION={version_integer}", "-C", "{0}/backend/ecu_simulation/DoorsModule".format(PROJECT_SRC_PATH)], check=True)
            doors_executable_path = "{0}/backend/ecu_simulation/DoorsModule/main_doors".format(
                PROJECT_SRC_PATH)
        ...
        if sw_to_build == "hvac" or sw_to_build == "all":
            if "clean" in extra_args:
                subprocess.run(
                    ["sudo", "make", "clean", "-C", "{0}/backend/ecu_simulation/HVACModule".format(PROJECT_SRC_PATH)], check=True)
            # Build the hvac Module executable
            subprocess.run(
                ["sudo", "make", f"SOFTWARE_VERSION={version_integer}", "-C", "{0}/backend/ecu_simulation/HVACModule".format(PROJECT_SRC_PATH)], check=True)
            hvac_executable_path = "{0}/backend/ecu_simulation/HVACModule/main_hvac".format(
                PROJECT_SRC_PATH)
